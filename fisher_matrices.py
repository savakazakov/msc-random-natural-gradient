...
class Quantum_Fisher_Information_Matrix:
    ...
    def QFIM(
        ...
        if which_parameters == "all":
            ...
            qfim = 4 * np.real(
                QGT.run(quantum_circuit, [values], [parameters_to_calculate_qgt])
                .result()
                .qgts[0]
            )
...
class Classical_Fisher_Information_Matrix:
    ...
    def construct_cfim(
        ...
        probabilities = (
            sampler.run([quantum_circuit], [parameters]).result().quasi_dists[0]
        )

        gradient = ParamShiftSamplerGradient(sampler)
        gradients_result = (
            gradient.run([quantum_circuit], [parameters]).result().gradients[0]
        )
            ...
            for parameter2 in range(self.number_of_parameters):
                if parameter1 <= parameter2:
                    ...
                    for outcome in outcomes:
                        try:
                            classical_fisher[parameter1, parameter2] += (
                                (1 / probabilities[outcome])
                                * derivatives1[outcome]
                                * derivatives2[outcome]
                            )
                        except:
                            pass

                else:
                    classical_fisher[parameter1, parameter2] = classical_fisher[
    ...
    def choose_measurement_basis(self, basis, options):
        ...
        if basis == "x":
            ...
            qcirc &= TwoLocal(
                num_qubits=self.number_of_qubits,
                rotation_blocks=options["single_qubit_gates"],
                entanglement_blocks=options["entanglement_gates"],
