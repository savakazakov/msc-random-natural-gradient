...
class Optimizers:
    def __init__(
        ...
        if self.ansatz == "HEA":
            self.quantum_circuit = TwoLocal(
                num_qubits=self.number_of_qubits,
                rotation_blocks=self.single_qubit_gates,
                entanglement_blocks=self.entanglement_gates,
    ...
    def expectation_value(self, angles, shots=None):
        ...
        exp_value = (
            estimator.run(self.quantum_circuit, self.hamiltonian, angles)
            .result()
            .values[0]
        )
    ...
    def calculate_derivatives(self, angles, which_derivatives, shots=None):
        ...
        derivatives = (
            gradient.run(
                ...
                self.hamiltonian,
                [angles],
                [parameters_to_calculate_gradient],
            )
            .result()
            .gradients[0]
        )
    ...
    def random_natural_gradient(
        ...
        for iteration in range(self.maxiter):
            ...
            CFIM = Classical_Fisher_Information_Matrix(
                self.number_of_qubits, self.number_of_parameters
            )

            cfim = CFIM.construct_cfim(
                ansatz=self.ansatz,
                basis=basis,
                options=options,
                parameters=thetas,
            ...
            if basis == "random":
                if exp_value < initial_exp_value:
                    print(
                        f"Random Natural Gradient: Iteration {iteration}, Expectation Value {exp_value}"
                    )
                    exp_values.append(exp_value)
                    ...
                    print(
                        f"Random Natural Gradient: Iteration {iteration}, Expectation Value {initial_exp_value}"
                    )
    ...
    def quantum_natural_gradient(self, eta=0.1, rcond=10**-4):
        ...
        for iteration in range(self.maxiter):
            ...
            quantum_fisher_information_matrix = QFIM.QFIM(
                ansatz=self.ansatz,
                values=thetas,
                single_qubit_gates=self.single_qubit_gates,
                entanglement_gates=self.entanglement_gates,
            ...
            inverse_qfim = np.linalg.pinv(
                quantum_fisher_information_matrix, rcond=rcond, hermitian=True
            )
            ...
            print(
                f"Quantum Natural Gradient: Iteration {iteration}, Expectation Value {exp_value}"
            )
            exp_values.append(exp_value)

        return exp_values

    def stochastic_quantum_natural_gradient(
        self, parameters_to_sample, rcond=10**-4, eta=0.01, shots=None
    ):

        thetas = self.angles.copy()
        print(
            "We begin the optimization using Stochastic-Coordinate Quantum Natural Gradient"
        )
        print(f"with initial expectation value {self.initial_exp_value}")
        exp_values = [self.initial_exp_value]
        initial_exp_value = self.initial_exp_value
        ...
        for iteration in range(self.maxiter):
            ...
            inverse_reduced_qfim = np.linalg.pinv(
                reduced_qfim, rcond=rcond, hermitian=True
            )
            new_thetas = thetas.copy()

            for j in range(len(indices)):
                for k in range(len(indices)):
                    new_thetas[indices[j]] -= (
                        eta * inverse_reduced_qfim[j, k] * derivatives[k]
                    )
            ...
            if exp_value < initial_exp_value:
                print(
                    f"Stochastic-Coordinate Quantum Natural Gradient: Iteration {iteration}, Expectation Value {exp_value}"
                )
                exp_values.append(exp_value)
                initial_exp_value = exp_value
                thetas = new_thetas
                ...
                print(
                    f"Stochastic-Coordinate Quantum Natural Gradient: Iteration {iteration}, Expectation Value {initial_exp_value}"
                )
                exp_values.append(initial_exp_value)
